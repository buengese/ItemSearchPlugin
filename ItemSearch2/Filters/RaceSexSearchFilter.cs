...
using static ItemSearch2.ClassExtensions;

namespace ItemSearch2.Filters {
    internal class RaceSexSearchFilter : SearchFilter {
        private int selectedOption;
        private int lastIndex;
        private readonly List<(string text, uint raceId, ClassExtensions.CharacterSex sex)> options;
        ...
        public RaceSexSearchFilter() {
            ...
            options = new List<(string text, uint raceId, ClassExtensions.CharacterSex sex)> {
                (Loc.Localize("NotSelected", "Not Selected"), 0, ClassExtensions.CharacterSex.Female)
            };
            ...
            foreach (var race in Service.Data.GetExcelSheet<Race>().ToList()) {
                if (race.RSEMBody.Row > 0 && race.RSEFBody.Row > 0) {
                    ...
                    options.Add((male, race.RowId, ClassExtensions.CharacterSex.Male));
                    options.Add((female, race.RowId, ClassExtensions.CharacterSex.Female));
                } else if (race.RSEMBody.Row > 0) {
                    options.Add((race.Masculine, race.RowId, ClassExtensions.CharacterSex.Male));
                } else if (race.RSEFBody.Row > 0) {
                    options.Add((race.Feminine, race.RowId, ClassExtensions.CharacterSex.Female));
                }
            }
        }
            ...
            if (Service.ClientState?.LocalContentId != 0) {
                ...
                if (ImGui.SmallButton($"Current")) {
                    if (Service.ClientState?.LocalPlayer != null) {
                        ...
                        var sex = Service.ClientState.LocalPlayer.Customize[(int)CustomizeIndex.Gender] == 0 ? ClassExtensions.CharacterSex.Male : ClassExtensions.CharacterSex.Female;
